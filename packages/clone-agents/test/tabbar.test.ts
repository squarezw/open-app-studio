...
import { DEMO_SHOP_APP, DEMO_TABBED_APP, FakeDriver } from '@oas/device-bridge';
...
describe('tabbar-aware exploration (tabbed demo app)', () => {
  it('tags pre-main screens, marks the tabbar, and visits every tab', async () => {
    ...
    expect(byTitle.get('Home Feed')?.phase).toBe('main');

    // the main screen carries a detected tabbar pattern
    expect(byTitle.get('Home Feed')?.patterns?.some((p) => p.kind === 'tabbar')).toBe(true);
