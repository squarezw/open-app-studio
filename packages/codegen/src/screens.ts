...
export function screenFile(screen: ScreenSpec): string {
  const refs = [...new Set(screen.components.map((c) => componentName(c.ref)))].sort();
  ...
  const imports = [
    ...
    `import { ${refs.join(', ')} } from '../components/oas';`,
